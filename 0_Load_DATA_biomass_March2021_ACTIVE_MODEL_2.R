...
# Transient
TRANSIENT %>% rm()
TRANSIENT <- all.data %>% filter(Larval_behaviour == "active" & ModelMode == "transi25") %>% droplevels()
...
# Parental
PARENTAL %>% rm()
PARENTAL <- all.data %>% filter(Larval_behaviour == "active" & ModelMode == "pare15") %>% droplevels()
...
# Cryptic
CRYPTIC %>% rm()
CRYPTIC <- all.data %>% filter(Larval_behaviour == "active" & ModelMode == "crypto15") %>% droplevels()
...
# Resident
RESID %>% rm()
RESID <- all.data %>% filter(Larval_behaviour == "active" & ModelMode == "resid25") %>% droplevels()
