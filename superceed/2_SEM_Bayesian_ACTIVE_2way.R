...
species_mod_inflow.intr <- bf(Richness ~ temp + log_grav_total*Class + Class*Netflow  + 
...
species_mod_inflow.intr.extr <- bf(Richness ~ temp + log_grav_total*Class+ Class*Netflow+ 
                                log_InflowBR +log_SelfR + log_btwdegree + 
...
species_mod_inflow.intr.extr.simp <- bf(Richness ~ temp + log_grav_total*Class+ Class*Netflow+ 
...
biom_mod_inflow.intr <- bf(log_biomassarea ~ Richness + temp + log_grav_total*Class+ Class*Netflow  + 
                             log_InflowBR +log_SelfR + log_btwdegree +  
                             (1+ log_grav_total + log_InflowBR +log_SelfR + log_btwdegree+ Netflow |region))

rm(biom_mod_inflow.intr.extr) # ENV + CON
biom_mod_inflow.intr.extr <- bf(log_biomassarea ~ temp + log_grav_total*Class+ Class*Netflow + 
                                  log_InflowBR +log_SelfR + log_btwdegree + 
                                  log_grav_neiBR + log_InflowMPABR + log_InflowNeiBR +
                                  log_CorridorIndegreeBR + (1+ log_grav_total + log_InflowBR +log_SelfR + log_btwdegree + log_grav_neiBR + log_InflowMPABR + log_InflowNeiBR +
                                                              log_CorridorIndegreeBR + Netflow |region))
...
biom_mod_inflow.intr.extr.simp <- bf(log_biomassarea ~ Richness + temp + log_grav_total*Class+ Class*Netflow+ 
                                       log_InflowBR +log_SelfR +  
                                       log_grav_neiBR +  log_InflowNeiBR  + 
                                       (1+ log_grav_total + log_InflowBR +log_SelfR +  log_grav_neiBR + log_InflowNeiBR  + Netflow |region))
...
#TRANSIENT total
all_fit_brms.tot.TRANSIENT.intr.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.TRANSIENT.intr.2way <-brm(species_mod_inflow.intr + biom_mod_inflow.intr + set_rescor(FALSE), data=TRANSIENT.std,cores=4,chains = 4,
...
saveRDS(all_fit_brms.tot.TRANSIENT.intr.2way,"all_fit_brms.tot.TRANSIENT.intr.2way.Rds")
all_fit_brms.tot.TRANSIENT.intr.extr.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.TRANSIENT.intr.extr.2way <-brm(species_mod_inflow.intr.extr + biom_mod_inflow.intr.extr + set_rescor(FALSE), data=TRANSIENT.std,cores=4,chains = 4,
                                      iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                      prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.TRANSIENT.intr.extr.2way,"all_fit_brms.tot.TRANSIENT.intr.extr.2way.Rds")

all_fit_brms.tot.TRANSIENT.intr.extr.simp.2way %>% rm()  # Simple - connectivity through both S and B
all_fit_brms.tot.TRANSIENT.intr.extr.simp.2way <-brm(species_mod_inflow.intr.extr.simp + biom_mod_inflow.intr.extr.simp + set_rescor(FALSE), data=TRANSIENT.std,cores=4,chains = 4,
                                           iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                           prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.TRANSIENT.intr.extr.simp.2way,"all_fit_brms.tot.TRANSIENT.intr.extr.simp.2way.Rds")


#TRANSIENT species mediated
all_fit_brms.nocon.S.TRANSIENT.intr.2way %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.TRANSIENT.intr.2way <-brm(species_mod_inflow.intr + biom_mod_nocon.S + set_rescor(FALSE), data=TRANSIENT.std,cores=4,chains = 4,
                                     iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                     prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.TRANSIENT.intr.2way,"all_fit_brms.nocon.S.TRANSIENT.intr.2way.Rds")

all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way <-brm(species_mod_inflow.intr.extr + biom_mod_nocon.S + set_rescor(FALSE), data=TRANSIENT.std,cores=4,chains = 4,
                                          iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                          prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way,"all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way.Rds")

#
all_fit_brms.nocon.TRANSIENT.2way %>% rm() # no connectivity for S and B, only environment
all_fit_brms.nocon.TRANSIENT.2way <-brm(S_mod_nocon + biom_mod_nocon.S + set_rescor(FALSE), data=TRANSIENT.std,cores=4,chains = 4,
                                  iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                  prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.TRANSIENT.2way,"all_fit_brms.nocon.TRANSIENT.2way.Rds")


TRANSIENT.weight <- model_weights(all_fit_brms.tot.TRANSIENT.intr.2way,
                                  all_fit_brms.tot.TRANSIENT.intr.extr.2way,
                                  all_fit_brms.nocon.S.TRANSIENT.intr.2way,
                                  all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way,
                                  all_fit_brms.tot.TRANSIENT.intr.extr.simp.2way,
                                  all_fit_brms.nocon.TRANSIENT.2way,weights = "loo")

TRANSIENT.LOO <- LOO(all_fit_brms.tot.TRANSIENT.intr.2way,
                     all_fit_brms.tot.TRANSIENT.intr.extr.2way,
                     all_fit_brms.nocon.S.TRANSIENT.intr.2way,
                     all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way,
                     all_fit_brms.tot.TRANSIENT.intr.extr.simp.2way,
                     all_fit_brms.nocon.TRANSIENT.2way)
WAIC(all_fit_brms.tot.TRANSIENT.intr.2way,
     all_fit_brms.tot.TRANSIENT.intr.extr.2way,
     all_fit_brms.nocon.S.TRANSIENT.intr.2way,
     all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way,
     all_fit_brms.tot.TRANSIENT.intr.extr.simp.2way,
...
TRANSIENT.R2 <- rbind(bayes_R2(all_fit_brms.tot.TRANSIENT.intr.2way),
bayes_R2(all_fit_brms.tot.TRANSIENT.intr.extr.2way),
bayes_R2(all_fit_brms.nocon.S.TRANSIENT.intr.2way),
bayes_R2(all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way),
bayes_R2(all_fit_brms.tot.TRANSIENT.intr.extr.simp.2way),
bayes_R2(all_fit_brms.nocon.TRANSIENT.2way))


mcmc_plot(all_fit_brms.tot.TRANSIENT.intr.2way)
mcmc_plot(all_fit_brms.tot.TRANSIENT.intr.extr.2way)
mcmc_plot(all_fit_brms.nocon.S.TRANSIENT.intr.2way)
mcmc_plot(all_fit_brms.nocon.S.TRANSIENT.intr.extr.2way)
mcmc_plot(all_fit_brms.nocon.TRANSIENT.2way)

describe_posterior(all_fit_brms.nocon.TRANSIENT.2way)
conditional_effects(all_fit_brms.tot.TRANSIENT.intr.2way)
...
posterior.TRANSIENT.tot <- as.array(all_fit_brms.tot.TRANSIENT.intr.extr.2way)
posterior.TRANSIENT.simp <- as.array(all_fit_brms.simp.TRANSIENT.2way)

# RESID
all_fit_brms.tot.RESID.intr.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.RESID.intr.2way <-brm(species_mod_inflow.intr + biom_mod_inflow.intr + set_rescor(FALSE), data=RESID.std,cores=4,chains = 4,
                                      iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                      prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.RESID.intr.2way,"all_fit_brms.tot.RESID.intr.2way.Rds")


all_fit_brms.tot.RESID.intr.extr.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.RESID.intr.extr.2way <-brm(species_mod_inflow.intr.extr + biom_mod_inflow.intr.extr + set_rescor(FALSE), data=RESID.std,cores=4,chains = 4,
                                           iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                           prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.RESID.intr.extr.2way,"all_fit_brms.tot.RESID.intr.extr.2way.Rds")

all_fit_brms.tot.RESID.intr.extr.simp.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.RESID.intr.extr.simp.2way <-brm(species_mod_inflow.intr.extr.simp + biom_mod_inflow.intr.extr.simp + set_rescor(FALSE), data=RESID.std,cores=4,chains = 4,
                                                iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                                prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.RESID.intr.extr.simp.2way,"all_fit_brms.tot.RESID.intr.extr.simp.2way.Rds")


#RESID species mediated
all_fit_brms.nocon.S.RESID.intr.2way %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.RESID.intr.2way <-brm(species_mod_inflow.intr + biom_mod_nocon.S + set_rescor(FALSE), data=RESID.std,cores=4,chains = 4,
                                          iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                          prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.RESID.intr.2way,"all_fit_brms.nocon.S.RESID.intr.2way.Rds")

all_fit_brms.nocon.S.RESID.intr.extr.2way %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.RESID.intr.extr.2way <-brm(species_mod_inflow.intr.extr + biom_mod_nocon.S + set_rescor(FALSE), data=RESID.std,cores=4,chains = 4,
                                               iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                               prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.RESID.intr.extr.2way,"all_fit_brms.nocon.S.RESID.intr.extr.2way.Rds")

#
all_fit_brms.nocon.RESID.2way %>% rm() # no connectivity for S and B, only environment
all_fit_brms.nocon.RESID.2way <-brm(S_mod_nocon + biom_mod_nocon.S + set_rescor(FALSE), data=RESID.std,cores=4,chains = 4,
                                   iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                   prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.RESID.2way,"all_fit_brms.nocon.RESID.2way.Rds")


RESID.weight <- model_weights(all_fit_brms.tot.RESID.intr.2way,
                                  all_fit_brms.tot.RESID.intr.extr.2way,
                                  all_fit_brms.nocon.S.RESID.intr.2way,
                                  all_fit_brms.nocon.S.RESID.intr.extr.2way,
                                  all_fit_brms.tot.RESID.intr.extr.simp.2way,
                                  all_fit_brms.nocon.RESID.2way,weights = "loo")

RESID.LOO <- LOO(all_fit_brms.tot.RESID.intr.2way,
                     all_fit_brms.tot.RESID.intr.extr.2way,
                     all_fit_brms.nocon.S.RESID.intr.2way,
                     all_fit_brms.nocon.S.RESID.intr.extr.2way,
                     all_fit_brms.tot.RESID.intr.extr.simp.2way,
                     all_fit_brms.nocon.RESID.2way)
WAIC(all_fit_brms.tot.RESID.intr.2way,
     all_fit_brms.tot.RESID.intr.extr.2way,
     all_fit_brms.nocon.S.RESID.intr.2way,
     all_fit_brms.nocon.S.RESID.intr.extr.2way,
     all_fit_brms.tot.RESID.intr.extr.simp.2way,
...
RESID.R2 <- rbind(bayes_R2(all_fit_brms.tot.RESID.intr.2way),
                      bayes_R2(all_fit_brms.tot.RESID.intr.extr.2way),
                      bayes_R2(all_fit_brms.nocon.S.RESID.intr.2way),
                      bayes_R2(all_fit_brms.nocon.S.RESID.intr.extr.2way),
                      bayes_R2(all_fit_brms.tot.RESID.intr.extr.simp.2way),
                      bayes_R2(all_fit_brms.nocon.RESID.2way))

mcmc_plot(all_fit_brms.tot.RESID.intr.2way)
mcmc_plot(all_fit_brms.tot.RESID.intr.extr.2way)
mcmc_plot(all_fit_brms.nocon.S.RESID.intr.2way)
mcmc_plot(all_fit_brms.nocon.S.RESID.intr.extr.2way)
mcmc_plot(all_fit_brms.nocon.RESID.2way)

# PARENTAL
all_fit_brms.tot.PARENTAL.intr.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.PARENTAL.intr.2way <-brm(species_mod_inflow.intr + biom_mod_inflow.intr + set_rescor(FALSE), data=PARENTAL.std,cores=4,chains = 4,
                                      iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                      prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.PARENTAL.intr.2way,"all_fit_brms.tot.PARENTAL.intr.2way.Rds")

all_fit_brms.tot.PARENTAL.intr.extr.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.PARENTAL.intr.extr.2way <-brm(species_mod_inflow.intr.extr + biom_mod_inflow.intr.extr + set_rescor(FALSE), data=PARENTAL.std,cores=4,chains = 4,
                                           iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                           prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.PARENTAL.intr.extr.2way,"all_fit_brms.tot.PARENTAL.intr.extr.2way.Rds")

all_fit_brms.tot.PARENTAL.intr.extr.simp.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.PARENTAL.intr.extr.simp.2way <-brm(species_mod_inflow.intr.extr.simp + biom_mod_inflow.intr.extr.simp + set_rescor(FALSE), data=PARENTAL.std,cores=4,chains = 4,
                                                iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                                prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.PARENTAL.intr.extr.simp.2way,"all_fit_brms.tot.PARENTAL.intr.extr.simp.2way.Rds")


#PARENTAL species mediated
all_fit_brms.nocon.S.PARENTAL.intr.2way %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.PARENTAL.intr.2way <-brm(species_mod_inflow.intr + biom_mod_nocon.S + set_rescor(FALSE), data=PARENTAL.std,cores=4,chains = 4,
                                          iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                          prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.PARENTAL.intr.2way,"all_fit_brms.nocon.S.PARENTAL.intr.2way.Rds")


all_fit_brms.nocon.S.PARENTAL.intr.extr.2way %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.PARENTAL.intr.extr.2way <-brm(species_mod_inflow.intr.extr + biom_mod_nocon.S + set_rescor(FALSE), data=PARENTAL.std,cores=4,chains = 4,
                                               iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                               prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.PARENTAL.intr.extr.2way,"all_fit_brms.nocon.S.PARENTAL.intr.extr.2way.Rds")

#
all_fit_brms.nocon.PARENTAL.2way %>% rm() # no connectivity for S and B, only environment
all_fit_brms.nocon.PARENTAL.2way <-brm(S_mod_nocon + biom_mod_nocon.S + set_rescor(FALSE), data=PARENTAL.std,cores=4,chains = 4,
                                   iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                   prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.PARENTAL.2way,"all_fit_brms.nocon.PARENTAL.2way.Rds")


PARENTAL.weight <- model_weights(all_fit_brms.tot.PARENTAL.intr.2way,
                                  all_fit_brms.tot.PARENTAL.intr.extr.2way,
                                  all_fit_brms.nocon.S.PARENTAL.intr.2way,
                                  all_fit_brms.nocon.S.PARENTAL.intr.extr.2way,
                                  all_fit_brms.tot.PARENTAL.intr.extr.simp.2way,
                                  all_fit_brms.nocon.PARENTAL.2way,weights = "loo")

PARENTAL.LOO <- LOO(all_fit_brms.tot.PARENTAL.intr.2way,
                     all_fit_brms.tot.PARENTAL.intr.extr.2way,
                     all_fit_brms.nocon.S.PARENTAL.intr.2way,
                     all_fit_brms.nocon.S.PARENTAL.intr.extr.2way,
                     all_fit_brms.tot.PARENTAL.intr.extr.simp.2way,
                     all_fit_brms.nocon.PARENTAL.2way)
WAIC(all_fit_brms.tot.PARENTAL.intr.2way,
     all_fit_brms.tot.PARENTAL.intr.extr.2way,
     all_fit_brms.nocon.S.PARENTAL.intr.2way,
     all_fit_brms.nocon.S.PARENTAL.intr.extr.2way,
     all_fit_brms.tot.PARENTAL.intr.extr.simp.2way,
...
PARENTAL.R2 <- rbind(bayes_R2(all_fit_brms.tot.PARENTAL.intr.2way),
                      bayes_R2(all_fit_brms.tot.PARENTAL.intr.extr.2way),
                      bayes_R2(all_fit_brms.nocon.S.PARENTAL.intr.2way),
                      bayes_R2(all_fit_brms.nocon.S.PARENTAL.intr.extr.2way),
                      bayes_R2(all_fit_brms.tot.PARENTAL.intr.extr.simp.2way),
                      bayes_R2(all_fit_brms.nocon.PARENTAL.2way))

mcmc_plot(all_fit_brms.tot.PARENTAL.intr.2way)
mcmc_plot(all_fit_brms.tot.PARENTAL.intr.extr.2way)
mcmc_plot(all_fit_brms.nocon.S.PARENTAL.intr.2way)
mcmc_plot(all_fit_brms.nocon.S.PARENTAL.intr.extr.2way)
mcmc_plot(all_fit_brms.nocon.PARENTAL.2way)

# CRYPTIC
all_fit_brms.tot.CRYPTIC.intr.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.CRYPTIC.intr.2way <-brm(species_mod_inflow.intr + biom_mod_inflow.intr + set_rescor(FALSE), data=CRYPTIC.std,cores=4,chains = 4,
                                      iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                      prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.CRYPTIC.intr.2way,"all_fit_brms.tot.CRYPTIC.intr.2way.Rds")


all_fit_brms.tot.CRYPTIC.intr.extr.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.CRYPTIC.intr.extr.2way <-brm(species_mod_inflow.intr.extr + biom_mod_inflow.intr.extr + set_rescor(FALSE), data=CRYPTIC.std,cores=4,chains = 4,
                                           iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                           prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.CRYPTIC.intr.extr.2way,"all_fit_brms.tot.CRYPTIC.intr.extr.2way.Rds")

all_fit_brms.tot.CRYPTIC.intr.extr.simp.2way %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.CRYPTIC.intr.extr.simp.2way <-brm(species_mod_inflow.intr.extr.simp + biom_mod_inflow.intr.extr.simp + set_rescor(FALSE), data=CRYPTIC.std,cores=4,chains = 4,
                                                iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                                prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))

saveRDS(all_fit_brms.tot.CRYPTIC.intr.extr.simp.2way,"all_fit_brms.tot.CRYPTIC.intr.extr.simp.2way.Rds")

#CRYPTIC species mediated
all_fit_brms.nocon.S.CRYPTIC.intr.2way %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.CRYPTIC.intr.2way <-brm(species_mod_inflow.intr + biom_mod_nocon.S + set_rescor(FALSE), data=CRYPTIC.std,cores=4,chains = 4,
                                          iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                          prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.CRYPTIC.intr.2way,"all_fit_brms.nocon.S.CRYPTIC.intr.2way.Rds")

all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way <-brm(species_mod_inflow.intr.extr + biom_mod_nocon.S + set_rescor(FALSE), data=CRYPTIC.std,cores=4,chains = 4,
                                               iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                               prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way,"all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way.Rds")

#
all_fit_brms.nocon.CRYPTIC.2way %>% rm() # no connectivity for S and B, only environment
all_fit_brms.nocon.CRYPTIC.2way <-brm(S_mod_nocon + biom_mod_nocon.S + set_rescor(FALSE), data=CRYPTIC.std,cores=4,chains = 4,
                                   iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                   prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.CRYPTIC.2way,"all_fit_brms.nocon.CRYPTIC.2way.Rds")


CRYPTIC.weight <- model_weights(all_fit_brms.tot.CRYPTIC.intr.2way,
                                  all_fit_brms.tot.CRYPTIC.intr.extr.2way,
                                  all_fit_brms.nocon.S.CRYPTIC.intr.2way,
                                  all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way,
                                  all_fit_brms.tot.CRYPTIC.intr.extr.simp.2way,
                                  all_fit_brms.nocon.CRYPTIC.2way,weights = "loo")

CRYPTIC.LOO <- LOO(all_fit_brms.tot.CRYPTIC.intr.2way,
                     all_fit_brms.tot.CRYPTIC.intr.extr.2way,
                     all_fit_brms.nocon.S.CRYPTIC.intr.2way,
                     all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way,
                     all_fit_brms.tot.CRYPTIC.intr.extr.simp.2way,
                     all_fit_brms.nocon.CRYPTIC.2way)
WAIC(all_fit_brms.tot.CRYPTIC.intr.2way,
     all_fit_brms.tot.CRYPTIC.intr.extr.2way,
     all_fit_brms.nocon.S.CRYPTIC.intr.2way,
     all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way,
     all_fit_brms.tot.CRYPTIC.intr.extr.simp.2way,
...
CRYPTIC.R2 <- rbind(bayes_R2(all_fit_brms.tot.CRYPTIC.intr.2way),
                      bayes_R2(all_fit_brms.tot.CRYPTIC.intr.extr.2way),
                      bayes_R2(all_fit_brms.nocon.S.CRYPTIC.intr.2way),
                      bayes_R2(all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way),
                      bayes_R2(all_fit_brms.tot.CRYPTIC.intr.extr.simp.2way),
                      bayes_R2(all_fit_brms.nocon.CRYPTIC.2way))

mcmc_plot(all_fit_brms.tot.CRYPTIC.intr.2way)
mcmc_plot(all_fit_brms.tot.CRYPTIC.intr.extr.2way)
mcmc_plot(all_fit_brms.nocon.S.CRYPTIC.intr.2way)
mcmc_plot(all_fit_brms.nocon.S.CRYPTIC.intr.extr.2way)
mcmc_plot(all_fit_brms.nocon.CRYPTIC.2way)
...
# TRANSIENT TOT
a <- mcmc_intervals(all_fit_brms.tot.TRANSIENT.intr.extr.2way)
...
Richness_inflow_TRANSIENT_tot.2way <- mcmc_intervals(all_fit_brms.tot.TRANSIENT.intr.extr.2way, pars = as.character(rich.var))
Biomass_inflow_TRANSIENT_tot.2way <- mcmc_intervals(all_fit_brms.tot.TRANSIENT.intr.extr.2way,pars = as.character(biom.var))
...
Richness_inflow_TRANSIENT_tot_data.2way <- mcmc_intervals_data(all_fit_brms.tot.TRANSIENT.intr.extr.2way, pars = as.character(rich.var))
Biomass_inflow_TRANSIENT_tot_data.2way <- mcmc_intervals_data(all_fit_brms.tot.TRANSIENT.intr.extr.2way,pars = as.character(biom.var))
