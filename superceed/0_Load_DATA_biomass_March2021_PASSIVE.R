...
# Transient
TRANSIENT %>% rm()
TRANSIENT <- all.data %>% filter(Larval_behaviour == "passive") %>% droplevels()
...
# Parental
PARENTAL %>% rm()
PARENTAL <- all.data %>% filter(Larval_behaviour == "passive") %>% droplevels()
...
# Cryptic
CRYPTIC %>% rm()
CRYPTIC <- all.data %>% filter(Larval_behaviour == "passive") %>% droplevels()
...
# Resident
RESID %>% rm()
RESID <- all.data %>% filter(Larval_behaviour == "passive") %>% droplevels()
