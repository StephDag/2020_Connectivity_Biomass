...
species_mod_inflow.intr.extr <- bf(Richness ~ temp + prod.annual + log_grav_total+Class+Netflow  + log_Indegree+
                                     log_SelfR + log_btwdegree +log_grav_neiBR + log_Indegree_Neigh +log_CorridorIn + 
                                     (1+   temp + prod.annual + log_grav_total + log_Indegree+
                                        log_SelfR + log_btwdegree +log_grav_neiBR + log_Indegree_Neigh +log_CorridorIn |region))

#### SPECIES RICHNESS = Environment + Connectivity simplified
rm(species_mod_inflow.intr.extr.simp) # ENV + CON simplified
species_mod_inflow.intr.extr.simp <- bf(Richness ~ prod.annual + log_grav_total+Class+Netflow  + log_Indegree+
                                     log_SelfR + log_btwdegree +log_grav_neiBR + log_Indegree_Neigh +log_CorridorIn + 
                                     (1+   prod.annual + log_grav_total+ log_Indegree+
                                        log_SelfR + log_btwdegree +log_grav_neiBR + log_Indegree_Neigh +log_CorridorIn |region))

#### SPECIES RICHNESS = Environment
rm(S_mod_nocon) # ENV
S_mod_nocon <- bf(Richness ~ prod.annual + log_grav_total+Class  + (1+ temp + prod.annual+ log_grav_total|region))

#### BIOMASS = Environment + Connectivity
rm(biom_mod_inflow.intr.extr) # ENV + CON
biom_mod_inflow.intr.extr <- bf(log_biomassarea ~ Richness +temp + prod.annual + log_grav_total*Class*Netflow  + log_Indegree+
                                  log_SelfR + log_btwdegree +log_grav_neiBR + log_Indegree_Neigh +log_CorridorIn + 
                                  (1+   temp + prod.annual + log_grav_total*Class*Netflow  + log_Indegree+
                                     log_SelfR + log_btwdegree +log_grav_neiBR + log_Indegree_Neigh +log_CorridorIn |region))

#### BIOMASS = Environment + Connectivity simplified
rm(biom_mod_inflow.intr.extr.simp) # ENV + CON simplified
biom_mod_inflow.intr.extr.simp <- bf(log_biomassarea ~ Richness +temp + log_grav_total*Class*Netflow  +log_Indegree+
                                  log_SelfR + log_btwdegree +log_grav_neiBR + log_CorridorIn + 
                                  (1+   temp + log_grav_total*Class*Netflow  + log_Indegree+
...
all_fit_brms.tot.TRANSIENT.intr.extr <-brm(species_mod_inflow.intr.extr + biom_mod_inflow.intr.extr + set_rescor(FALSE), data=TRANSIENT.std,cores=10,chains = 4,
...
saveRDS(all_fit_brms.tot.TRANSIENT.intr.extr,"ACTIVE_models/Indegree/all_fit_brms.tot.TRANSIENT.intr.extr_INDEGREE.Rds")
...
all_fit_brms.nocon.S.TRANSIENT.intr.extr <-brm(species_mod_inflow.intr.extr.simp + biom_mod_nocon.S + set_rescor(FALSE), data=TRANSIENT.std,cores=10,chains = 4,
                                          iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                          prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.TRANSIENT.intr.extr,"ACTIVE_models/Indegree/all_fit_brms.nocon.S.TRANSIENT.intr.extr_INDEGREE.Rds")

# TRANSIENT no connectivity
all_fit_brms.nocon.TRANSIENT %>% rm() # no connectivity for S and B, only environment
all_fit_brms.nocon.TRANSIENT <-brm(S_mod_nocon + biom_mod_nocon.S + set_rescor(FALSE), data=TRANSIENT.std,cores=10,chains = 4,
                                  iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                  prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.TRANSIENT,"ACTIVE_models/Indegree/all_fit_brms.nocon.TRANSIENT_INDEGREE.Rds")

# RESIDENT
  # Full
all_fit_brms.tot.RESID.intr.extr %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.RESID.intr.extr <-brm(species_mod_inflow.intr.extr + biom_mod_inflow.intr.extr + set_rescor(FALSE), data=RESID.std,cores=10,chains = 4,
                                           iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                           prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.RESID.intr.extr,"ACTIVE_models/Indegree/all_fit_brms.tot.RESID.intr.extr_INDEGREE.Rds")
...
#                                                iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
#                                                prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
#saveRDS(all_fit_brms.tot.RESID.intr.extr.simp,"ACTIVE_models/Indegree/all_fit_brms.tot.RESID.intr.extr.simp_INFLOW.Rds")

  # species mediated
all_fit_brms.nocon.S.RESID.intr.extr %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.RESID.intr.extr <-brm(species_mod_inflow.intr.extr.simp + biom_mod_nocon.S + set_rescor(FALSE), data=RESID.std,cores=10,chains = 4,
                                               iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                               prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.RESID.intr.extr,"ACTIVE_models/Indegree/all_fit_brms.nocon.S.RESID.intr.extr_INDEGREE.Rds")

# no connectivity
all_fit_brms.nocon.RESID %>% rm() # no connectivity for S and B, only environment
all_fit_brms.nocon.RESID <-brm(S_mod_nocon + biom_mod_nocon.S + set_rescor(FALSE), data=RESID.std,cores=10,chains = 4,
                                   iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                   prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.RESID,"ACTIVE_models/Indegree/all_fit_brms.nocon.RESID_INDEGREE.Rds")

# PARENTAL
  # full
all_fit_brms.tot.PARENTAL.intr.extr %>% rm()  # Full - connectivity through both S and B
all_fit_brms.tot.PARENTAL.intr.extr <-brm(species_mod_inflow.intr.extr + biom_mod_inflow.intr.extr + set_rescor(FALSE), data=PARENTAL.std,cores=10,chains = 4,
                                           iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                           prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.tot.PARENTAL.intr.extr,"ACTIVE_models/Indegree/all_fit_brms.tot.PARENTAL.intr.extr_INDEGREE.Rds")
...
#                                               iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
#                                                prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
#saveRDS(all_fit_brms.tot.PARENTAL.intr.extr.simp,"ACTIVE_models/Indegree/all_fit_brms.tot.PARENTAL.intr.extr.simp_INFLOW.Rds")

  # species mediated
all_fit_brms.nocon.S.PARENTAL.intr.extr %>% rm() # connectivity only through S + ENV
all_fit_brms.nocon.S.PARENTAL.intr.extr <-brm(species_mod_inflow.intr.extr.simp + biom_mod_nocon.S + set_rescor(FALSE), data=PARENTAL.std,cores=10,chains = 4,
                                               iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                               prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.S.PARENTAL.intr.extr,"ACTIVE_models/Indegree/all_fit_brms.nocon.S.PARENTAL.intr.extr_INDEGREE.Rds")

# no connectivity
all_fit_brms.nocon.PARENTAL %>% rm() # no connectivity for S and B, only environment
all_fit_brms.nocon.PARENTAL <-brm(S_mod_nocon + biom_mod_nocon.S + set_rescor(FALSE), data=PARENTAL.std,cores=10,chains = 4,
                                   iter = 5000, warmup = 1000,thin = 2, refresh = 0, control = list(adapt_delta = 0.99999,max_treedepth = 30),
                                   prior = c(prior(normal(0, 100),class = "Intercept"), prior(normal(0, 100), class = "b")))
saveRDS(all_fit_brms.nocon.PARENTAL,"ACTIVE_models/Indegree/all_fit_brms.nocon.PARENTAL_INDEGREE.Rds")
